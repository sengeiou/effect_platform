...
public class HoloTreeBuilder {
	...
	private static final Logger log = LoggerFactory
			.getLogger(HoloTreeBuilder.class);

	// 全局字符串常量池最大尺寸，按单个URL 1k计，可能占用内存已达 3GB，要小心更改！
	private static final int MAX_STRING_POOL_SIZE = 3000000;
	...
	private static final String[] LOGIN_REDIRECT_URL_KEYS = { "redirectURL",
			"redirect_url", "tpl_redirect_url", "redirectUrl",
			"TPL_redirect_url" };

	// key/val 均引用同一 String 对象（注意是同一对象！）的全局字符串常量池，仅实时应用场景下使用
	// XXX:
	// (qingwu)暂时用Collections.synchronizedMap()包装粗粒度锁进行同步，锁竞争成为瓶颈后可考虑拆分为多个子结构并使用读写锁！
	@SuppressWarnings("unchecked")
	private static final Map<String, String> strConstsPool = Collections
			.synchronizedMap(new LRUMap(MAX_STRING_POOL_SIZE));
	 ...
	 *            建树事件处理器
	 */
	public HoloTreeBuilder(HoloConfig conf, boolean autoCompact,
			List<HoloTreeProcessor> procs) {
		this.conf = conf;
	...
	private HoloTreeNode attachNodeByRefer(HoloTreeNode node) {
		...
		logEntry.put("refer_revised", false);
		...
		if (url.startsWith(LOGIN_REDIRECT_URL_PREFIX)) {
			...
			for (String redirectKey : LOGIN_REDIRECT_URL_KEYS) {
				if (queryMap.containsKey(redirectKey)) {
					try {
						String redirectURL = URLDecoder.decode(
								queryMap.get(redirectKey), "utf-8");
	...
	private void srcPathMatch(HoloTreeNode curNode) {
		...
		/*
		 * 优化：仅当 curNode 的 pType 或 rType 在用户定义路径效果页类型集合中存在时才执行匹配操作
		 */
		if (exactPTypeMatch || exactRTypeMatch) {
			...
			if (exactPTypeMatch && pType != 0) {
				...
				// 识别出了“其他来源”
				curNode.addSource(OTHER_SRC_PATH_FEATURE, new SourceMeta(
						curNode));
			}
	 ...
	 * @param curTree
	 */
	private void processInternalNode(HoloTreeNode prevNode,
			HoloTreeNode curNode, HoloTree curTree) {
		PTLogEntry logEntry = curNode.getPtLogEntry();
		long ts = (Long) logEntry.get("ts");
		// 将日志中的秒级时戳同 session 内节点序号整合，以避免时戳重复影响建树
		...
		// 设置序号 root path
		curNode.setSerialRootPath(prevNode.getSerialRootPath() + "."
				+ String.valueOf(curSessSerial));
		...
		if (rType != 0 && rType != prevPType) {
			// 需要用当前节点的 pType 替换 root path 中的父节点页面类型
	...
	private void processRootNode(HoloTreeNode curNode, HoloTree curTree) {
		...
		// 设置序号 root path，树根节点的 root path 只有一个字段
	...
	private void adjustReferByLoginNode(HoloTreeNode curNode) {
		...
		try {
			...
			e.printStackTrace();
			System.out.println("ts:" + String.valueOf(logEntry.get("ts"))
					+ "  url:" + url + "  refer_url:"
					+ (String) logEntry.get("refer_url") + "  acookie:"
					+ (String) logEntry.get("acookie") + "  session_cookie:"
					+ (String) logEntry.get("session_cookie") + "  lzsession:"
					+ (String) logEntry.get("lzsession"));
		}
		...
		if (loginNode != null) {
			...
			// refer修复标志
			logEntry.put("refer_revised", true);
		}
	}
	 ...
	 * 
	 * TODO: 吸星大法 browse_log 记录的 refer_url 已经经过了一次 escape，导致 http:// 被转换为
	 * http%3A//，无法同 refer 修正算法直接匹配，但 HoloTreeUtil.unescape 又进行了双重 unescape，可能导致
	 * s.taobao.com 的搜索关键词等参数解析错误，如何处理才能同离线给出的 refer_url 一致？
	 * 
	 * @param logEntry
	 */
	private void fixupReferAndUrl(PTLogEntry logEntry) {
		...
		if (autoCompact) {
			if (strConstsPool.containsKey(fixedRefer)) {
				fixedRefer = strConstsPool.get(fixedRefer);
			} else {
				strConstsPool.put(fixedRefer, fixedRefer);
			}

			if (strConstsPool.containsKey(fixedUrl)) {
				fixedUrl = strConstsPool.get(fixedUrl);
			} else {
				strConstsPool.put(fixedUrl, fixedUrl);
			}
		}
	 ...
	 * @param logEntry
	 */
	private void removeSharp(PTLogEntry logEntry) {
		String refer = (String) logEntry.get("refer_url");
		String url = (String) logEntry.get("url");

		if (refer.indexOf('#') != -1) {
			logEntry.put("refer_url", refer.substring(0, refer.indexOf('#')));
		}
		if (url.indexOf('#') != -1) {
			logEntry.put("url", url.substring(0, url.indexOf('#')));
		}
