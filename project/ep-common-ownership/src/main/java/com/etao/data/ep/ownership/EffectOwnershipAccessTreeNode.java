 ...
 * 代表了在一个方案下的一条浏览日志
 */
public class EffectOwnershipAccessTreeNode extends EffectOwnershipNode {
	
	public EffectOwnershipAccessTreeNode(TreeNodeValue value){
		super(value);
	...
	public void setPlanPathInfo(TypePathInfo t_path_info) {
		...
		if ( path_infos.size() == 0 ) {
			...
			/*
			 * first - 归属至从源头开始首个来源(即离效果发生处最远的来源) 
			 * last  - 归属至从源头开始最后一个来源(即离效果发生处最近的来源) 
			 * equal - 所有踩中的来源均分效果 
			 * all   - 所有踩中的来源同时得到相同效果
			 */
