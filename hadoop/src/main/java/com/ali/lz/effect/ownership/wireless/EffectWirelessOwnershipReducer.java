...
public class EffectWirelessOwnershipReducer extends MapReduceBase implements
     ...
     *            被归属结点
     */
    private void outputValue(String target, OutputCollector<Text, Text> output, WirelessNodeValue owner,
            WirelessNodeValue value) {

        WirelessNodeValue.Builder builder = LzEffectWirelessProtoUtil.genBuilder(owner);
        ...
        try {
            String data = LzEffectWirelessProtoUtil.toString(builder.build());
     ...
     * @return
     */
    private HashMap<String, HashMap<String, WirelessNodeValue[]>> updateMap(String nodeType, String flag,
            WirelessNodeValue value, HashMap<String, HashMap<String, WirelessNodeValue[]>> planMap) {

        String IDKey;
        WirelessNodeValue[] nodeList;
        HashMap<String, WirelessNodeValue[]> nodeMap;
        // map中区分宝贝页和店铺页的标记key
        if (flag.equals("i")) {
        ...
        // map中已存在相应记录，直接替换更新
        if (planMap.containsKey(value.getPlanId()) && planMap.get(value.getPlanId()).containsKey(IDKey)) {

            nodeMap = planMap.get(value.getPlanId());
            ...
            planMap.put(value.getPlanId(), nodeMap);

        } else { // 新记录，map结构中插入新记录
            nodeList = new WirelessNodeValue[2];
            if (planMap.containsKey(value.getPlanId())) {
                nodeMap = planMap.get(value.getPlanId());
            } else
                nodeMap = new HashMap<String, WirelessNodeValue[]>();
     ...
     * @param output
     */
    private void effectOwnership(String effectType, WirelessNodeValue value,
            HashMap<String, HashMap<String, WirelessNodeValue[]>> planMap, OutputCollector<Text, Text> output,
            Reporter reporter) {

        String keyID;
        WirelessNodeValue tempNode;
        WirelessNodeValue tempNode1;
        WirelessNodeValue tempNode2;
        if (planMap.isEmpty()) {
        ...
        for (HashMap<String, WirelessNodeValue[]> nodemap : planMap.values()) {
    ...
    public void reduce(TextPair key, Iterator<BytesWritable> values, OutputCollector<Text, Text> output,
        ...
        HashMap<String, HashMap<String, WirelessNodeValue[]>> planMap = new HashMap<String, HashMap<String, WirelessNodeValue[]>>();

        while (values.hasNext()) {
            BytesWritable nodeData = values.next();
            WirelessNodeValue value = LzEffectWirelessProtoUtil.deserializeWirelessNodeValue(Arrays.copyOf(
