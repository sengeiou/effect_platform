...
/**
 * 匹配路径规则集合类，驱动各条路径规则的全息树匹配操作，并根据路径规则优先级调整匹配结果。
 * 
 * @author minzhan
 * 
 */
public class RuleSet {
     ...
     * 依据配置文件，生成规则对象集合
     */
    private void generateRules(HoloConfig conf) {
    ...
    /**
     * 匹配TreeNode, 如果所有路径匹配完成或者所有均无法匹配成功，返回false，否则true
     * 
     * @param treeNode
     * @return
     */
    public boolean matchNext(HoloTreeNode treeNode) {
        ...
        for (Rule r : ruleListToMatch) {
            ...
            /**
             * 由于从 HoloConfig 获得的规则列表是按优先级从高到低排序的，因此若当前规则的优先级低于已匹配成功规则的优先级，
             * 就意味着后续规则优先级全都低于该优先级，不需匹配可直接返回
             * 注意：priority值越小表示规则优先级越高
             */
