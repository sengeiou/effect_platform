...
//    private HoloConfig.PathRule pathRuleConf;
    ...
    // 保存规则各级节点对应的 expand 展开值（逆序，沿路径配置从后向前排列）
...
//        this.pathRuleConf = pathRuleConf;
        prepare(pathRuleConf);
    }

    protected void prepare(HoloConfig.PathRule pathRuleConf) {
        ...
        for (; i >= 0; i--) {
            ...
            RuleNode ruleNode = new RuleNode(child.type_refs, child.expand, tmpSkip);
    ...
    public boolean matchNext(HoloTreeNode treeNode) {
        ...
        if (skip == -1) {
            ...
            // 效果页折叠
            if (folder) {
                if (effectPType != 0 && (effectPType == pType || effectPType == effectRType)) {
                    ...
                    if (effectSkip == 1) {
                        ...
                        if (curTypeIDs.contains(effectRType) || typeIDs.contains(-1)) {

                            expand(logEntry, effectRType, curRuleNode.getExpand());
                            if (curRuleNode.isOwnerPoint()) {
                                opNode = treeNode;
                            }
    ...
    /**
     * 添加规则各级节点对应的 expand 展开值（逆序，沿路径配置从后向前排列）
     * @param curLogEntry
     * @param typeID
     * @param expand
     */
    ...
    /**
     * 生成路径颜色
     * 格式为：pathID\002expand1\003expand2\003expand3....
     */
