...
# Date: 2012-11-21
# ------------------------------------------------------------------------
# Description: etao频道效果 联盟广告来源明细表
...
$Hive <<EOF
drop table lz_rpt_ep_etao_channel_pid;

create external table lz_rpt_ep_etao_channel_pid
(
   day                                  string comment '日期',
   channel_id                           bigint comment '频道id',
   pub_id                               string comment 'pub id',
   pid_site_id                              string comment '网站id',
   site_name                            string comment '网站名称',
   adzone_id                            string comment '广告位id',
   adzone_name                          string comment '广告位名称',
   adzone_size                          string comment '广告位尺寸',
   adzone_distype                       string comment '广告位展示类型',
...
location '${hive_rpt_dir}/lz_rpt_ep_etao_channel_pid';
alter table lz_rpt_ep_etao_channel_pid set serdeproperties('serialization.null.format' = '');
