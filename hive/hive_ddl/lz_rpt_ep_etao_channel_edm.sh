...
$Hive <<EOF
drop table lz_rpt_ep_etao_channel_edm;

create external table lz_rpt_ep_etao_channel_edm
(
   day                                  string comment '日期',
   channel_id                           bigint comment '频道id',
   apply                                string comment '通道名称',
   t_id                                 string comment '邮件任务名称',
   linkname                             string comment '链接名称',
...
location '${hive_rpt_dir}/lz_rpt_ep_etao_channel_edm';
alter table lz_rpt_ep_etao_channel_edm set serdeproperties('serialization.null.format' = '');
