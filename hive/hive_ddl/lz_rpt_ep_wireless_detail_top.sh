...
$Hive <<EOF
drop table lz_rpt_ep_wireless_detail_top;
create external table lz_rpt_ep_wireless_detail_top
(
   day          string comment '日期',
   platform_id  bigint comment '无线平台类型：html5/wap',
   plan_id      bigint comment '活动id',
   pit_id       bigint comment '坑位id，表示宝贝、店铺和List三类坑位',
   pit_detail   string comment '根据坑位类型分为宝贝ID/店铺ID/搜索关键字',
...
location '${hive_rpt_dir}/lz_rpt_ep_wireless_detail_top';
