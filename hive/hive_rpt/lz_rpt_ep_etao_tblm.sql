...
-- # Onput:lz_rpt_ep_etao_tblm                                             
-- # ------------------------------------------------------------------------ 
-- # ChangeLog:
-- ########################################################################## 

insert overwrite table lz_rpt_ep_etao_tblm partition (dt='${date}')
select
    '${date}' as day
    ,case when site_id='' or site_id is null then '-' else site_id end as site_id
    ,case when ad_id='' or ad_id is null then '-' else ad_id end as ad_id 
    ...
    ,count(distinct case when refer_is_lp=1 and direct_item_alipay_trade_num>0 then user_id end) as lp_direct_item_alipay_uv
    ...
    ,sum(case when refer_is_lp=1 
        then direct_itemshop_alipay_trade_num else 0.0 end) as lp_direct_itemshop_alipay_trade_num
...
where dt='${date}' and lp_src=200 and length(ad_id) < 50 
group by 
    case when site_id='' or site_id is null then '-' else site_id end 
    ,case when ad_id='' or ad_id is null then '-' else ad_id end 
    ,case when lp_domain_name='' or lp_domain_name is null 
    then '-' else lp_domain_name end 
;
