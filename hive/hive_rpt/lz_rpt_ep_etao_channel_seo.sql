...
-- # Date:2012/09/17
-- # ------------------------------------------------------------------------ 
-- # Description:产出etao频道seo表
-- # Input:lz_fact_ep_etao_channel_ownership
-- # Onput:lz_rpt_ep_etao_channel_seo                                             
-- # ------------------------------------------------------------------------ 
-- # ChangeLog:
-- ########################################################################## 


insert overwrite table lz_rpt_ep_etao_channel_seo partition (dt='${date}')
select
    '${date}' as day
    ,channel_id
...
(
    select
        ...
        ,ref_channel_keyword as keyword
        ,ref_channel_src_domain_name_level1 as src_domain_name_level1
    ...
    where dt='${date}' and refer_is_channel_lp=1 and refer_channel_src=203
    ...
    select
        '${date}' as day
        ,channel_id
        ,channel_keyword as keyword
        ,channel_src_domain_name_level1 as src_domain_name_level1
    ...
    from lz_fact_ep_etao_channel_ownership 
    where dt='${date}' and channel_id > 0 and channel_src=203
) a
group by 
    channel_id
    ,src_domain_name_level1
