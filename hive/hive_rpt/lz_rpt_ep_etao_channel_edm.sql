...
-- # Date:2012/09/17
-- # ------------------------------------------------------------------------ 
-- # Description:产出etao频道edm表
-- # Input:lz_fact_ep_etao_channel_ownership
-- # Onput:lz_rpt_ep_etao_channel_edm                                             
-- # ------------------------------------------------------------------------ 
-- # ChangeLog:
-- ########################################################################## 


insert overwrite table lz_rpt_ep_etao_channel_edm partition (dt='${date}')
select
    '${date}' as day
    ,channel_id
    ,apply
...
(
    select
        ...
        ,ref_channel_apply as apply
        ,case 
            when ref_channel_t_id='' or ref_channel_t_id is null then '-' 
            else ref_channel_t_id end as t_id
        ,case 
            when ref_channel_linkname='' or ref_channel_linkname is null then '-' 
            else ref_channel_linkname end as linkname
    ...
    where dt='${date}' and refer_is_channel_lp=1 and refer_channel_src=201 
    ...
    select
        '${date}' as day
        ,channel_id
        ,channel_apply as apply
        ,case 
            when channel_t_id='' or channel_t_id is null then '-' 
            else channel_t_id end as t_id
        ,case 
            when channel_linkname='' or channel_linkname is null then '-' 
            else channel_linkname end as linkname
    ...
    from lz_fact_ep_etao_channel_ownership 
    where dt='${date}' and channel_id > 0 and channel_src=201
) a
group by 
    channel_id
    ,apply
