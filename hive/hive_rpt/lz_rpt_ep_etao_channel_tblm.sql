...
-- # Date:2012/09/17
-- # ------------------------------------------------------------------------ 
-- # Description:产出etao频道内部合作来源表
-- # Input:lz_fact_ep_etao_channel_ownership
-- # Onput:lz_rpt_ep_etao_channel_tblm                                             
-- # ------------------------------------------------------------------------ 
-- # ChangeLog:
-- ########################################################################## 


insert overwrite table lz_rpt_ep_etao_channel_tblm partition (dt='${date}')
select
    '${date}' as day
    ,channel_id
    ,site_id
...
(
    select
        ...
        ,case 
            when ref_channel_site_id='' or ref_channel_site_id is null then '-' 
            else ref_channel_site_id end as site_id
        ,case 
            when ref_channel_ad_id='' or ref_channel_ad_id is null then '-' 
            else ref_channel_ad_id end as ad_id 
    ...
    where 
        ...
        and refer_channel_src=200
        and length(ref_channel_ad_id)<50
    ...
    select
        ...
        ,channel_id
        ,case 
            when channel_site_id='' or channel_site_id is null then '-' 
            else channel_site_id end as site_id
        ,case 
            when channel_ad_id='' or channel_ad_id is null then '-' 
            else channel_ad_id end as ad_id 
