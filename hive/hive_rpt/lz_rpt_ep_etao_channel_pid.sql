...
-- # Date:2012/09/17
-- # ------------------------------------------------------------------------ 
-- # Description:产出etao频道pid表
-- # Input:lz_fact_ep_etao_channel_ownership
-- # Onput:lz_rpt_ep_etao_channel_pid                                             
-- # ------------------------------------------------------------------------ 
-- # ChangeLog:
-- ########################################################################## 


insert overwrite table lz_rpt_ep_etao_channel_pid partition (dt='${date}')
select
   day                                  
   ,channel_id
   ,pub_id                               
   ...
   ,adzone_id                            
   ,coalesce(c.name, '-')                          
   ,coalesce(c.adzonesize, '-')                          
   ,coalesce(case when c.distype=1 then '固定广告' 
        when c.distype=2 then '对联广告' 
        when c.distype=3 then '右下浮层' 
        when c.distype=4 then '浮层视窗'
        when c.distype=5 then '弹窗广告'
        end, '-')
   ,effect_pv                            
   ,effect_uv                            
   ,effect_click_pv                      
...
(
    select
        ...
        ,case
            when ref_channel_pub_id='' or ref_channel_pub_id is null then '-'
            else ref_channel_pub_id end as pub_id
        ,case
            when ref_channel_pid_site_id='' or ref_channel_pid_site_id is null then '-'
            else ref_channel_pid_site_id end as pid_site_id
        ,case
            when ref_channel_adzone_id='' or ref_channel_adzone_id is null then '-'
            else ref_channel_adzone_id end as adzone_id
    ...
    where dt='${date}' and refer_is_channel_lp=1 and refer_channel_src=101 
    ...
    select
        ...
        ,channel_id
        ,case 
            when channel_pub_id='' or channel_pub_id is null then '-'
            else channel_pub_id end as pub_id
        ,case
            when channel_pid_site_id='' or channel_pid_site_id is null then '-'
            else channel_pid_site_id end as pid_site_id
        ,case
            when channel_adzone_id='' or channel_adzone_id is null then '-'
            else channel_adzone_id end as adzone_id
    ...
    from lz_fact_ep_etao_channel_ownership 
    where dt='${date}' and channel_id > 0 and channel_src=101
) a
group by 
    channel_id
    ,pub_id
...
left outer join s_ods_mm_adzones c
on c.ds='${date}' and b.pid_site_id=c.siteid and b.adzone_id=c.adzoneid
where effect_pv>0
;
