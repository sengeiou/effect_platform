...
-- # Date:2012/09/17
-- # ------------------------------------------------------------------------ 
-- # Description:产出etao edm来源明细表
-- # Input:lz_fact_ep_etao_ownership                                          
-- # Onput:lz_rpt_ep_etao_edm                                            
-- # ------------------------------------------------------------------------ 
-- # ChangeLog:
-- ########################################################################## 

insert overwrite table lz_rpt_ep_etao_edm partition (dt='${date}')
select
    '${date}' as day
    ,apply
    ,case 
        when t_id='' or t_id is null then '-' 
        else t_id end as t_id
    ,case 
        when linkname='' or linkname is null then '-' 
        else linkname end as linkname
    ,case 
        when lp_domain_name='' or lp_domain_name is null then '-' 
        else lp_domain_name end as lp_domain_name
    ...
    ,case 
        when count(distinct case when refer_is_lp=1 then cookie end)>0
    ...
    ,case 
        when count(distinct case when is_etao=1 then cookie end)>0 
...
where dt='${date}' and lp_src=201
group by 
    apply
    ,case when t_id='' or t_id is null then '-' else t_id end 
    ,case when linkname='' or linkname is null then '-' else linkname end 
