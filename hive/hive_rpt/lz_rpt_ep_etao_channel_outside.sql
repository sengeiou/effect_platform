...
-- # Date:2012/09/17
-- # ------------------------------------------------------------------------ 
-- # Description:产出etao频道站外自然流量表
-- # Input:lz_fact_ep_etao_channel_ownership
-- # Onput:lz_rpt_ep_etao_channel_outside                                             
-- # ------------------------------------------------------------------------ 
-- # ChangeLog:
-- ########################################################################## 


insert overwrite table lz_rpt_ep_etao_channel_outside partition (dt='${date}')
select
    '${date}' as day
    ,channel_id
...
(
    ...
    where dt='${date}' and refer_is_channel_lp=1 and refer_channel_src=204
    ...
    from lz_fact_ep_etao_channel_ownership 
    where dt='${date}' and channel_id > 0 and channel_src=204
) a
group by 
    channel_id
    ,src_domain_name_level1
