...
-- # Date:2012/09/17
-- # ------------------------------------------------------------------------ 
-- # Description:产出pid总览表
-- # Input:lz_fact_ep_etao_ownership                                          
-- # Onput:lz_rpt_ep_etao_pid                                            
-- # ------------------------------------------------------------------------ 
-- # ChangeLog:
-- ########################################################################## 

insert overwrite table lz_rpt_ep_etao_pid partition (dt='${date}')
select
   day                                  
   ,pub_id                               
   ,pid_site_id                              
   ...
   ,adzone_id                            
   ,coalesce(b.name, '-')                          
   ,coalesce(b.adzonesize, '-')                          
   ,coalesce(case when b.distype=1 then '固定广告' 
        when b.distype=2 then '对联广告' 
        when b.distype=3 then '右下浮层' 
        when b.distype=4 then '浮层视窗'
        when b.distype=5 then '弹窗广告'
        end, '-')
   ,lp_domain_name                       
   ,effect_pv                            
   ,effect_uv                            
...
where dt='${date}' and lp_src=101
group by
    pub_id
    ,pid_site_id
    ,adzone_id
    ,case when lp_domain_name='' or lp_domain_name is null then '-' else lp_domain_name end 
) a left outer join s_ods_mm_adzones b
on b.ds='${date}' and a.pid_site_id=b.siteid and a.adzone_id=b.adzoneid
where effect_pv>0
;
