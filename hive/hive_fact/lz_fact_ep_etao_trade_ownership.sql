...
-- # Input:lz_fact_ep_etao_cps_ownership, lz_fact_ep_etao_outside_ownership, lz_fact_ep_etao_inside_ownership
-- # Onput:lz_fact_ep_etao_channel_trade_ownership, lz_fact_ep_etao_trade_ownership
-- # ------------------------------------------------------------------------
-- # ChangeLog:
-- ##########################################################################
...
(
  ...
  from lz_fact_ep_etao_cps_ownership 
    where dt='${date}'
  ...
  select
    ...
    ,0.0 as cps_old_amt
  from lz_fact_ep_etao_outside_ownership
    where dt='${date}'
  ...
  select
    ...
    ,0.0 as cps_old_trade_num
    ,0.0 as cps_old_amt
    from lz_fact_ep_etao_inside_ownership
    where dt='${date}'
) a

insert overwrite table lz_fact_ep_etao_trade_ownership partition (dt='${date}')
select
   lp_src
  ,lp_domain_name
  ,refer_is_lp
